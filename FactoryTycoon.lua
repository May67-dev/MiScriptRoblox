...
FarmTab:Toggle({ Title = "Auto-Comprar Botones", Callback = function(s) AutoBuy = s end })
FarmTab:Toggle({ Title = "Auto-Rebirth (Glitch)", Callback = function(s) AutoRebirth = s end })
...
task.spawn(function()
    ...
    while true do
        ...
        if tycoon then
            ...
            -- 3. Auto Rebirth (El glitch del 184)
            if AutoRebirth then
                pcall(function() Events.RequestRebirth:FireServer(184, 184, tycoon) end)
            end
        end
        task.wait(0.5)
    end
