...
-- ==========================================
-- EFECTO ESPECIAL: BORDE RGB (ARCOÍRIS)
-- ==========================================
task.spawn(function()
    -- Esperamos a que la GUI cargue completamente
    task.wait(1)
    
    -- Buscamos la ventana de NC HUB en tu pantalla
    local ScreenGui = game.Players.LocalPlayer.PlayerGui:FindFirstChild("WindUI") or game.CoreGui:FindFirstChild("WindUI")
    
    if ScreenGui then
        local MainFrame = ScreenGui:FindFirstChild("Main") -- El cuadro principal del Hub
        
        if MainFrame then
            -- Creamos el borde (Stroke)
            local Stroke = Instance.new("UIStroke")
            Stroke.Thickness = 2.5 -- Grosor del borde
            Stroke.ApplyStrokeMode = Enum.ApplyStrokeMode.Border
            Stroke.LineJoinMode = Enum.LineJoinMode.Round
            Stroke.Parent = MainFrame
            
            -- Bucle infinito para el efecto arcoíris
            while true do
                for i = 0, 1, 0.005 do
                    Stroke.Color = Color3.fromHSV(i, 0.8, 1) -- Cambia el color suavemente
                    task.wait(0.02) -- Velocidad del cambio
                end
            end
        end
    end
